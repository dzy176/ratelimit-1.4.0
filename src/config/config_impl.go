...
// 对比配置文件, 查询传入descriptor是否命中已有限速规则, 命中则返回具体规则详情, 否则返回nil
func (this *rateLimitConfigImpl) GetLimit(
	...
	// descriptorsMap: 限速规则集合, 加载配置阶段已经通过递归将yaml文件转化成一个类似树状结构的数据
	// 只需要将descriptors数组中的k-v不断拼接, 沿着树由向叶子节点查找
	descriptorsMap := value.Descriptors

	// 这里可以调试打印 descriptorsMap 具体内容
	//dB, _ := json.MarshalIndent(descriptorsMap, "  ", " ")
	//fmt.Printf("%s\n", string(dB))
